...
const SummaryPop = ({ results, onRestart, onHome }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-2 landscape:p-1 md:p-4 md:landscape:p-2 lg:p-4 relative bg-[#d8e9fa]">
      {showConfetti && <Confetti />}

      {/* Fullscreen Button */}
      <button
        onClick={toggleFullscreen}
        className="fixed top-2 right-2 landscape:top-1 landscape:right-1 md:top-3 md:right-3 lg:top-4 lg:right-4 z-50 p-2 md:p-3 lg:p-3 rounded-full bg-[#b4d7ff] hover:bg-[#9fc9ff] transition-all shadow-lg"
      ...
      >
        ...
        <div className="text-center mb-2 landscape:mb-1 md:mb-4 md:landscape:mb-2 lg:mb-4 shrink-0">
          <h2 className="text-2xl landscape:text-lg md:text-3xl md:landscape:text-2xl lg:text-4xl font-bold text-[#3e366b] mb-1 landscape:mb-0 md:mb-2 lg:mb-2">
          ...
          {resultsData && resultsData.length > 0 ? (
            displayedResults.map((item, idx) => {
              ...
              return (
                ...
                >
                  <span className="text-lg landscape:text-sm md:text-2xl md:landscape:text-xl lg:text-2xl font-bold text-gray-400 w-10 landscape:w-8 md:w-14 lg:w-14 text-center shrink-0">
                    {item.questionNumber || idx + 1}
                  </span>
                  {item.correct ? (
        ...
        {showStats && (
          <div className="mascot-bounce-in mt-2 landscape:mt-1 md:mt-4 md:landscape:mt-2 lg:mt-4 shrink-0">
            {isCompetitionMode ? (
              <div className="text-center p-2 landscape:p-1.5 md:p-3 md:landscape:p-2 lg:p-3 rounded-xl landscape:rounded-lg md:rounded-2xl lg:rounded-2xl bg-gradient-to-r from-blue-100 to-sky-100 border-2 border-blue-300">
