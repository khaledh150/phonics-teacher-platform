    ...
    <div ref={containerRef} className="h-full w-full relative overflow-hidden flex flex-col"
      ...
      <div className="flex-1 flex flex-col landscape:flex-row items-center justify-center px-4 landscape:px-8 pt-10 md:pt-12 landscape:pt-14 min-h-0 gap-2 landscape:gap-8">
        ...
        >
          ...
          {/* Drag hint — fixed height wrapper so layout doesn't shift when it disappears */}
          <div style={{ minHeight: showHint && !blending && !wordDone ? 'auto' : 0 }}>
            <AnimatePresence>
              {showHint && !blending && !wordDone && (
                <motion.div exit={{ opacity: 0 }} className="z-30 pointer-events-none">
                  <DragHint />
                </motion.div>
              )}
            </AnimatePresence>
