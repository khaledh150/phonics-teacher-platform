        ...
        pixiAppRef.current._spawnBalloon = () => {
          ...
          const txt = new Text({
            ...
            style: new TextStyle({
              ...
              fontSize: Math.max(bSize * 0.45, 30),
              fontWeight: '900',
              fill: '#3e366b',
              stroke: { color: '#ffffff', width: 2.5 },
  ...
  return (
    <div ref={containerRef} className="h-full w-full relative overflow-hidden select-none">
      ...
      <div className="absolute top-0 left-0 right-0 z-50 flex items-center justify-between px-4 pt-3 md:pt-4 lg:pt-6">
        ...
        <motion.span
          className="text-base md:text-xl lg:text-2xl font-bold text-white/80 drop-shadow-lg"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
          Pop the Balloons!
      ...
      <AnimatePresence>
        {showTutorialOverlay && (
          ...
          >
            ...
            <motion.div
              className="relative z-10 bg-[#FFD000] px-3 py-1 md:px-5 md:py-2 rounded-full shadow-lg"
              style={{ border: '2px solid #3e366b', borderBottomWidth: '4px' }}
              animate={{ scale: [1, 1.05, 1] }}
              transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
            >
              <span className="text-[#3e366b] font-black text-sm md:text-base">How to Play!</span>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
