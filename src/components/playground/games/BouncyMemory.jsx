...
import { playWrongSfx } from '../../../utils/sfx';
    ...
    async (cardId) => {
      ...
      if (newFlippedIds.length === 2) {
        ...
        if (first.pairId === second.pairId) {
          ...
          // No match — shake + wrong SFX, then flip back
          playWrongSfx();
          await delay(1000);
          if (!mountedRef.current) return;
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-gradient-to-b from-[#1a1147] to-[#6B3FA0] overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center p-2 md:p-3">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 lg:gap-4 max-w-sm md:max-w-4xl lg:max-w-5xl w-full" style={{ maxHeight: '85vh' }}>
          {cards.map((card) => (
            <div
              key={card.id}
