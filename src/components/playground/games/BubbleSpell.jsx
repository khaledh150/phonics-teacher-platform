...
const BubbleSpellGame = ({ group, onBack, onPlayAgain }) => {
        ...
        app.ticker.add((ticker) => {
          ...
          for (let i = bubbles.length - 1; i >= 0; i--) {
            ...
            if (b.y < -R * 2) {
              ...
              const isLand = cW > cH;
              b.x = isLand ? cW * 0.25 + Math.random() * (cW * 0.5) : cW * 0.3 + Math.random() * (cW * 0.4);
              b.growPhase = 0;
            }
  ...
  useEffect(() => {
    ...
    const spawnBubble = (letter) => {
      ...
      // Spawn from behind the dog — in landscape, narrow the spread to match dog width
      const isLandscape = w > h;
      const spreadMin = isLandscape ? 0.2 : 0.1;
      const spreadRange = isLandscape ? 0.6 : 0.8;
      const startX = w * spreadMin + Math.random() * (w * spreadRange);
  ...
  if (gameComplete) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-gradient-to-b from-[#5BA3D9] to-[#87CEEB]">
        <GameResultCard
          title="Spelling Star!"
          subtitle="You spelled all the words!"
          accentColor="#4ECDC4"
          icon={Droplets}
