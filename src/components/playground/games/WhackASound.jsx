  ...
  return (
    <div className="h-screen w-screen overflow-hidden relative flex flex-col bg-gradient-to-b from-[#1a1147] to-[#6B3FA0]">
      ...
      {gameComplete && (
        <GameResultCard
          title="Whack Master!"
          subtitle="You found all the sounds!"
          accentColor="#F59E0B"
          icon={Hammer}
      ...
      <div className="flex-1 flex flex-col items-center justify-center pt-20">
        ...
        <div className="grid grid-cols-3 gap-x-4 gap-y-3 md:gap-x-8 md:gap-y-5 lg:gap-x-12 lg:gap-y-7">
          {holes.map((hole, holeIndex) => (
            <div key={holeIndex} className="relative" style={{ width: 'clamp(130px, min(28vw, 20vh), 176px)', height: 'clamp(100px, min(22vw, 16vh), 144px)' }}>
              {/* The popping letter */}
              <AnimatePresence>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-0">
        <div className="h-10 md:h-14 landscape:h-8 bg-gradient-to-b from-[#22C55E] to-[#16A34A] rounded-t-[2rem]" />
        <div className="h-6 md:h-8 landscape:h-4 bg-[#16A34A]" />
      </div>
    </div>
  );
};
